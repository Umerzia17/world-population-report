...
public class PopulationRepository
{
    ...
    {
        String sql = "SELECT c.Name, c.Population AS TotalPopulation, " + "COALESCE(SUM(city.Population), 0) AS PopulationInCities " + "FROM country c LEFT JOIN city ON city.CountryCode = c.Code " + "GROUP BY c.Code, c.Name, c.Population ORDER BY c.Name";
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql); ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next()) {
                ...
                reports.add(new PopulationReport(resultSet.getString("Name"), totalPopulation, populationInCities, populationInCitiesPercentage, populationNotInCities, populationNotInCitiesPercentage));
            }
        }
        return reports;
    }
    ...
    {
        String sql = "SELECT city.District AS Name, SUM(city.Population) AS TotalPopulation " + "FROM city GROUP BY city.District ORDER BY city.District";
        List<PopulationReport> reports = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(sql); ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next())
            {
                long totalPopulation = resultSet.getLong("TotalPopulation");
                reports.add(new PopulationReport(resultSet.getString("Name"), totalPopulation, 0L, 0.0, totalPopulation, 100.0));
            }
        }
        return reports;
    }
    ...
    {
        String sql = "SELECT Language, SUM(country.Population * countrylanguage.Percentage / 100) AS Speakers " + "FROM countrylanguage JOIN country ON countrylanguage.CountryCode = country.Code " + "WHERE Language IN ('Chinese', 'English', 'Spanish') " + "GROUP BY Language ORDER BY Speakers DESC";
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql); ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next())
            {
                ...
                reports.add(new LanguageReport(resultSet.getString("Language"), speakers, worldPercentage));
            }
        }
        return reports;
    }
    ...
    private List<PopulationReport> findGroupedPopulationByCountryField(String groupColumn) throws SQLException {
        String sql = "SELECT c." + groupColumn + " AS Name, SUM(c.Population) AS TotalPopulation, " + "COALESCE(SUM(city_population.PopulationInCities), 0) AS PopulationInCities " + "FROM country c " + "LEFT JOIN (SELECT CountryCode, SUM(Population) AS PopulationInCities FROM city GROUP BY CountryCode) city_population " + "ON city_population.CountryCode = c.Code " + "GROUP BY c." + groupColumn + " ORDER BY c." + groupColumn;
        List<PopulationReport> reports = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(sql); ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next())
            {
